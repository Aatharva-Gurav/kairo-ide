...
export function BinaryFileView({ document }: BinaryFileViewProps) {
  ...
  if (isImage) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center p-8 bg-background select-none overflow-auto">
        <div className="rounded-lg border border-border bg-card/60 p-4 shadow-sm max-w-lg flex flex-col items-center">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={imageSrc}
            alt={document.title}
            className="max-h-[60vh] max-w-full object-contain rounded border border-border/50"
          />
          <div className="mt-3 flex items-center justify-between w-full text-xs text-muted-foreground font-mono">
            <span>{document.title}</span>
            <Button size="xs" variant="outline" onClick={handleReveal}>
              <HugeiconsIcon icon={Folder01Icon} className="size-3 mr-1" />
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center p-8 text-center select-none bg-background">
      <div className="flex size-14 items-center justify-center rounded-2xl bg-amber-500/10 text-amber-500 mb-4 border border-amber-500/20">
